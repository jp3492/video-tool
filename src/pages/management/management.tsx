...
export const Management = (props: any) => {
  ...
  const [selectedFolderId, setSelectedFolderId] = quantumState({
    id: "SELECTED_FOLDER_ID",
    initialValue: ""
  });
  const [editingFolder, setEditingFolder] = useState();

  const handleEditingFolder = useCallback(
    folder =>
  ...
  const handleAddProjectId = useCallback(
    _id =>
      selectedProjectIds.includes(_id)
            ...
            if (target.type === "project") {
              ...
              if (thisProject.links.every(l => l.url !== link)) {
                ...
                patchProject({
                  ...thisProject,
                  _id: target.id,
                  links: [...thisProject.links, { url: link, label }]
                });
              } else {
  ...
  return (
    <div className="management">
      <div data-sidebar-open={sideBarOpen} className="management__sidebar">
        {/* <i
          onClick={() => openSideBar(!sideBarOpen)}
          className="material-icons">
          chevron_right
        </i> */}
        <a onClick={e => folderModal()} className="button">
      ...
      <div className="management__content">
        <div className="management__content__files">
          <div className="management__content__files-header">
            ...
            />
            <a onClick={() => projectModal()} className="button">
              <i className="material-icons">add</i>
          ...
          <ul
            onDragEnter={e =>
              handleUrlDrop(e, { type: "folder", id: selectedFolderId })
            }
            onDragOver={e =>
              handleUrlDrop(e, { type: "folder", id: selectedFolderId })
            }
          ...
          >
            {filteredProjects.map((p, i) => (
              <Project
                ...
                {...p}
                onDragEnter={e =>
                  handleUrlDrop(e, { type: "project", id: p._id })
                }
                onDragOver={e =>
                  handleUrlDrop(e, { type: "project", id: p._id })
                }
          ...
          <div>
            <div className="management__content__selection-header">
              <h4>
                {`${selectedProjectIds.length} Selected Project${
              ...
              {selectedProjectIds.length !== 0 && (
                <>
                  ...
                  <Link
                    to={`/player?ids=${JSON.stringify(selectedProjectIds)}`}
                  >
              ...
              {selectedProjectIds.map(id => {
                ...
                return (
                  <Project
                    {...thisProject}
                    onDragEnter={e =>
                      handleUrlDrop(e, { type: "project", id: thisProject._id })
                    }
                    onDragOver={e =>
                      handleUrlDrop(e, { type: "project", id: thisProject._id })
                    }
...
const Information = ({
  ...
  return (
    <div className="management__content__information">
      ...
      {selectedTab === "Details" ? <Details /> : <Access />}
      <ul className="management__content__information-footer">
        ...
        <li>
          <Link to={`/player?ids=${JSON.stringify([_id])}`}>
            <label className="button">Open</label>
          </Link>
        </li>
        <li>
  ...
  return (
    ...
    >
      <i
        ...
        className="material-icons"
      >
        {inSelection ? "check_box" : "check_box_outline_blank"}
      </i>
      <label>{label}</label>
    </li>
  );
