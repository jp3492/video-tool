...
export const Header = ({ logo, navItems, components }: iHeader) => {
  ...
  const mobileTabs = useMemo(() => {
    ...
    return [...nav, ...menu].map(i => ({
      ...i,
      selectTab,
      selected: selectedTab === i.label
    }));
  }, [navItems, components, selectedTab]);

  const DropComponentContent = useMemo(() => {
    const contentComponent =
      components && components.find(c => c.label === selectedTab);
    if (contentComponent) {
  ...
  return (
    <header id="header">
      ...
      >
        <ul className="header__drop-header">{mobileTabs.map(MobileTab)}</ul>
        <nav className="header__nav">
          <ul>{navItems && navItems.map(MenuItem)}</ul>
        </nav>
        <menu className="header__menu">
          <ul>{components && components.map(Component)}</ul>
        </menu>
...
const Component = ({ label, icon, headerComponent }: iComponent) => {
...
const MenuItem = ({ label, to, icon }: iMenuItem) => (
  <li key={label}>
    <Link to={to}>
      <i className="material-icons">{icon}</i>
      {/* <label>
