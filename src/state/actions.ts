...
const USER_ACTION = options =>
  ACTION(options)(action =>
    dispatchToStores({ ids: [REDUCERS.USERS], action })
  );
const FOLDER_ACTION = options =>
  ACTION(options)(action =>
    dispatchToStores({ ids: [REDUCERS.FOLDERS], action })
  );
const PROJECT_ACTION = options =>
  ACTION(options)(action =>
    dispatchToStores({ ids: [REDUCERS.PROJECTS], action })
  );
const TAG_ACTION = options =>
  ACTION(options)(action => dispatchToStores({ ids: [REDUCERS.TAGS], action }));
const REQUEST_ACTION = options =>
  ACTION(options)(action =>
    dispatchToStores({ ids: [REDUCERS.REQUESTS], action })
  );
const SEARCH_ACTION = options =>
  ACTION(options)(action =>
    dispatchToStores({ ids: [REDUCERS.SEARCH], action })
  );
...
export const patchFolder = async ({ _id, ...body }) =>
  await FOLDER_ACTION({
    ...requests.folders.patch,
    url: requests.folders.patch.url + _id,
    body
  });
export const deleteFolder = async _id =>
  await FOLDER_ACTION({
    ...requests.folders.delete,
    url: requests.folders.patch.url + _id
  });
...
export const patchProject = async ({ _id, ...body }) =>
  await PROJECT_ACTION({
    ...requests.projects.patch,
    url: requests.projects.patch.url + _id,
    body
  });
export const deleteProject = async _id =>
  await FOLDER_ACTION({
    ...requests.projects.delete,
    url: requests.projects.patch.url + _id
  });
...
const targets = (option, search) => {
  switch (option) {
    case 'user':
      return getStateById(REDUCERS.USERS).data.filter(({ email }) =>
        email.toLowerCase().includes(search)
      );
    case 'project':
      return getStateById(REDUCERS.PROJECTS).data.filter(({ label }) =>
        label.toLowerCase().includes(search)
      );
    case 'folder':
      return getStateById(REDUCERS.FOLDERS).data.filter(({ label }) =>
        label.toLowerCase().includes(search)
      );
    case 'tag':
      return getStateById(REDUCERS.TAGS).data.filter(({ text }) =>
...
const getLocalSearchResult = (search: string, options: any) => {
  const activeOptions: string[] = Object.keys(options).reduce(
    (res: any, o) => (options[o] ? [...res, o] : res),
    []
  );
  let result = {};
...
export const postSearch = async (options, search) => {
  if (options.global) {
    ...
    TAG_ACTION(requests.tags.get).then(() => {
      ...
      dispatchToStores({
        ids: [REDUCERS.SEARCH],
        action: { type: REQUEST_TYPES.GET, data }
      });
    });
  }
};
...
export const getTags = async projectIds =>
  await TAG_ACTION({
    ...requests.tags.get,
    url: requests.tags.get.url + projectIds
  });
export const postTag = async (projectId, tag) => {
  try {
    const newTag = await TAG_ACTION({
      ...requests.tags.post,
      url: requests.tags.post.url + projectId,
      body: tag
    });
...
export const patchTag = async tag =>
  await TAG_ACTION({
    ...requests.tags.patch,
    url: requests.tags.post.url + tag._id,
    body: tag
  });

export const removeTagsFromProject = async tagIds => {
  const projectId = JSON.parse(getQuantumValue(PLAYER_STATES.PROJECT_ID))[0];
  const thisProject = getItems(REDUCERS.PROJECTS).find(
    p => p._id === projectId
  );
  return await patchProject({
    ...thisProject,
    tags: thisProject.tags.filter(
      tag => !tagIds.map(({ _id }) => _id).includes(tag)
    )
  });
};

export const addTagToProject = (projectId, tagId) =>
  dispatchToStores({
...
// COMMENT
export const postComment = async (tagId, text) =>
  TAG_ACTION({
    ...requests.comments.post,
    url: requests.comments.post.url + tagId,
    body: { text }
  });
export const patchComment = async (tagId, commentId, text) =>
  TAG_ACTION({
    ...requests.comments.patch,
    url: requests.comments.patch.url + tagId + '/' + commentId,
    body: { text }
  });
export const deleteComment = async (tagId, commentId) =>
  TAG_ACTION({
    ...requests.comments.delete,
    url: requests.comments.delete.url + tagId + '/' + commentId
  });
// MODAL
export const folderModal = (initialValues?: any) =>
  openModal({
...
export const projectModal = (initialValues?: any) =>
  openModal({
    ...
    props: {
      ...
      selectedFolderId: getQuantumValue('SELECTED_FOLDER_ID'),
      action: project =>
        initialValues && initialValues.hasOwnProperty('_id')
