...
const getRestMethod = method =>
  method.includes('_')
    ? method.split('_')[0].toLowerCase()
    : method.toLowerCase();

let paginationStatus: any = {};

export const ACTION = (requestOptions: RequestOptions) => async dispatch => {
  let { method, resetPagination = false, ...requestProps } = requestOptions;
...
export const REDUCER_LIST = (
  ...
  const resourceIdName =
    options && options.resourceIdName ? options.resourceIdName : 'resourceId';
    ...
    switch (type) {
      ...
      case REQUEST_TYPES.PATCH:
        return state.data.map(item =>
          item[resourceIdName] === newData[resourceIdName] ? newData : item
        );
      case REQUEST_TYPES.PATCH_MULTIPLE:
        return mergeArrayByKey(resourceIdName, state.data, newData);
      case REQUEST_TYPES.DELETE:
        return state.data.filter(
          item => item[resourceIdName] !== newData[resourceIdName]
        );
      case REQUEST_TYPES.DELETE_MULTIPLE:
        return state.data.filter(
          item => !newData.includes(item[resourceIdName])
        );
      default:
        return state.data;
    }
    ...
    case REQUEST_TYPES.ADD_TAG_TO_PROJECT:
      return {
        ...
        data: state.data.map(project =>
          project._id === action.data.projectId
            ? { ...project, tags: [...project.tags, action.data.tagId] }
            : project
        )
      };
